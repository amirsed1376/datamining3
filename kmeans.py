...
def k_means_plots(df, centers):
    ...
    for i in range(len(columns)):
        for j in range(i + 1, len(columns)):
            ...
            plt.savefig("outs\\KMeans\\{}---{}.png".format(columns[i], columns[j]))
...
def best_k_means(df, k, sql_manager):
    ...
    df.to_sql(name="clusters", con=sql_manager.conn, if_exists="replace")
    center_df = DataFrame(k_means.cluster_centers_, columns=columns)
    center_df.to_sql(name="centers", con=sql_manager.conn, if_exists="replace")
...
def reduce_dimension_k_means(samples, k, sql_manager):
    ...
    k_means.fit(samples)
    samples["cluster"] = k_means.labels_
    samples.to_sql(name="reduce_dimension_clusters", con=sql_manager.conn, if_exists="replace")
    center_df = DataFrame(k_means.cluster_centers_)
    center_df.to_sql(name="reduce_dimension_centers", con=sql_manager.conn, if_exists="replace")
